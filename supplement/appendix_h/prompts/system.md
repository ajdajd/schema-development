...
The baseline is **Data Snapshot Metadata Schema v1.2**. Treat it as authoritative and frozen. Your task is only to assess whether this schema adequately covers the supplied data snapshot. Do not revise the schema or decide whether a proposed field should be accepted.
...
1. Consider the complete v1.2 schema, but do not inventory covered information.
2. Identify only critical or possibly critical reusable snapshot metadata that may lack adequate coverage.
3. For each gap, identify the closest existing fields using exact v1.2 schema paths. Use dot notation and `[]` for array items, such as `variables[].unit` or `dimensions[].category_groups[].categories`. A wording difference or synonym is not a gap.
...
- identify exact closest v1.2 schema paths, or return an empty list if none is relevant;
